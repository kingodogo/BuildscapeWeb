...
export default function App() {
  const [currentView, setCurrentView] = useState<'home' | 'report' | 'admin' | 'login' | 'register' | 'suggestions' | 'suggestion-form' | 'profile' | 'changelog' | 'wiki' | 'redeem'>('home');
  
  // Type-safe navigation wrapper
  const handleNavigate = (view: string) => {
    const validViews: Array<'home' | 'report' | 'admin' | 'login' | 'register' | 'suggestions' | 'suggestion-form' | 'profile' | 'changelog' | 'wiki' | 'redeem'> = 
      ['home', 'report', 'admin', 'login', 'register', 'suggestions', 'suggestion-form', 'profile', 'changelog', 'wiki', 'redeem'];
            ...
            if (isProduction) {
                ...
                if (e instanceof StorageError) {
                    if (e.code === 'NOT_CONFIGURED') {
                        ...
                        setDbError("Running in local mode. API routes require Vercel dev server. Use 'vercel dev' to test API locally.");
                    } else {
  ...
  return (
    <div className="h-screen bg-[#121212] flex flex-col font-sans overflow-hidden">
      <Navbar onNavigate={setCurrentView} currentUser={currentUser} onLogout={handleLogout} links={config.links} navbarIcon={config.navbarIcon} />
      ...
      <main className="flex-1 overflow-hidden relative">
        {currentView === 'home' && (
          <div className="h-full overflow-y-auto custom-scrollbar fade-in">
            <Hero 
                ...
                onSuggestionsClick={() => setCurrentView('suggestion-form')}
                config={config.hero}
                links={config.links}
            />
        ...
        {currentView === 'login' && (
           <div className="h-full overflow-y-auto custom-scrollbar">
             <Login onLogin={handleLogin} onError={handleLoginError} onNavigateRegister={() => setCurrentView('register')} />
             <Footer links={config.links} socialHandles={config.socialHandles} footerIcon={config.footerIcon} />
           </div>
        )}
        ...
        {currentView === 'register' && (
           <div className="h-full overflow-y-auto custom-scrollbar">
             <Register onLogin={handleLogin} onError={handleRegisterError} onNavigateLogin={() => setCurrentView('login')} />
             <Footer links={config.links} socialHandles={config.socialHandles} footerIcon={config.footerIcon} />
           </div>
        )}
        ...
        {currentView === 'suggestions' && (
          <div className="h-full overflow-y-auto custom-scrollbar fade-in">
            <Hero 
                onReportClick={() => setCurrentView('report')} 
                onSuggestionsClick={() => setCurrentView('suggestions')}
                config={config.hero}
                links={config.links}
            />
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {currentView === 'suggestion-form' && (
          <div className="h-full fade-in overflow-hidden">
             <SuggestionsForm 
                ...
                onCancel={() => setCurrentView('suggestions')} 
                mcVersions={config.mcVersions}
                modVersions={config.modVersions}
                onNotify={handleNotify}
                currentUser={currentUser}
        ...
        {currentView === 'admin' && !currentUser && (
             <div className="min-h-[60vh] flex items-center justify-center">
                 <div className="text-center">
                     ...
                     <button onClick={() => setCurrentView('login')} className="bg-green-700 text-white px-4 py-2 rounded">Go to Login</button>
                 </div>
             </div>
        )}
