...
async def run_test():
    ...
    try:
        ...
        # -> Try to find an alternative way to open the login form or scroll to reveal more elements.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Admin Panel Bug Management Interface').first).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError("Test case failed: Admin users cannot view, create, update, or delete bug records through the admin panel interface as expected. The bug management interface is not accessible or UI defects are present.")
